...
Before we begin, we use the `set.seed()` function in order to set a *seed* for `R`'s random number generator for replicability. It is generally
...
We see a sharp drop in the estimated test MSE between the linear and quadratic fits, but no clear improvement from using higher-order polynomials.
