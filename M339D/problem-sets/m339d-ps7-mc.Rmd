...
```{r}
spinner=c(-1, 0, 1)
all.spins=sample(spinner, 1000, replace=TRUE)
```
...
#many runs
nsim=10000
many.runs=sample(spinner, nsim, replace=TRUE)

mean(many.runs)

#making a functions for the many runs
spin<-function(){
 mean(sample(spinner, nsim, replace=TRUE)) 
}

nbatch=1000
all.batches=replicate(nbatch, spin())
...
support=c(-1, 0, 9)
pmf=c(1/2, 1/3, 1/6)
th.mean=sum(support*pmf)

sims.gamble=sample(support, nsim, replace=TRUE, prob=pmf)
mean(sims.gamble)
```
...
runs=rbinom(nsim, size=50, prob=0.4)
mean(runs)
```
